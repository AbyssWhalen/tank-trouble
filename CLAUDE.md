...
## 模块职责
- `config.js` —— 全局常量：格子尺寸、速度、地图档位(`MAZE_TIERS`)、模式→档位池(`TIER_POOL_BY_MODE`)、视口预留、键位表、颜色
...
- `player.js` —— 玩家：绑定坦克 + 键位 + 颜色（单回合实体；累计分外置在 main 的 `matchScores`）
- `ai.js` —— AI 控制器，接口与 input 对齐（阶段 6）
- `main.js` —— 主循环 + 状态机（菜单/对战/回合结算）；持有跨回合的 `matchScores` 与竞技场缩放 `arenaScale`
